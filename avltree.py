...
def calculateBalance2(AVLTree):
    if AVLTree.root == None:
        return None
    else:
        calculateBalanceWrapper(AVLTree.root)

def calculateBalanceWrapper(AVLNode):
    left = 0
    right = 0
    if AVLNode.rightnode != None:
        right = 1 + calculateBalanceWrapper(AVLNode.rightnode)
    if AVLNode.leftnode != None:
        left = 1 + calculateBalanceWrapper(AVLNode.leftnode)
    AVLNode.balanceFactor = left - right
    if AVLNode.rightnode == None:
        return right
    if AVLNode.leftnode == None:
        return left
...
def calculateBalanceR2(AVLTree,currentNode):
  ...
  if currentNode.leftnode==None:
    ...
    left=calculateBalanceR2(AVLTree,currentNode.leftnode)+1
